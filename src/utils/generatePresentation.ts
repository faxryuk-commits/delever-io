...
const LOGO_WHITE_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1930 521" style="height: 70px; width: auto;">
  <defs>
    <linearGradient id="pres-grad" x1="-278.91" y1="759.37" x2="-278.91" y2="249.6" gradientTransform="translate(539.04 765.27) scale(1 -1)" gradientUnits="userSpaceOnUse">
      <stop offset="0" stop-color="#afdbd9"/><stop offset="1" stop-color="#f3e9dd"/>
    </linearGradient>
  </defs>
  <path fill="url(#pres-grad)" d="M348.26,5.89h-176.26C80.31,5.89,5.97,80.44,5.97,172.4v176.76c0,91.96,74.34,166.51,166.03,166.51h176.26c91.7,0,166.03-74.55,166.03-166.51v-176.76c0-91.96-74.34-166.51-166.03-166.51Z"/>
...
export function generatePresentation(data: PresentationData): string {
  ...
  const personalizedTitle = brandName ? `${brandName} ×` : ''
  ...
  const styles = `
    <style>
      ...
      * { margin: 0; padding: 0; box-sizing: border-box; }
      ...
      .logo-container { display: flex; align-items: center; justify-content: center; gap: 16px; margin-bottom: 16px; }
      .brand-name { font-size: 48px; font-weight: 800; color: white; letter-spacing: -1px; }
...
<head>
  ...
  <title>${brandName ? `${brandName} × Delever` : 'Delever'} - ${t.tagline}</title>
  ${styles}
</head>
<body>
  <!-- 1. ОБЛОЖКА -->
  <div class="slide slide-cover">
    ...
    <div class="logo-container">
      ${personalizedTitle ? `<span class="brand-name">${personalizedTitle}</span>` : ''}
